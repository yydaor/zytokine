Behavior = {
    behaviorInit = function(self, direction)
        ...
        beholder.observe('CHANGE DIRECTION' .. self.id, function() 
    ...
    behaviorUpdate = function(self, dt)
        if self.can_move then
            ...
            if self.p.x >= ENEMY_WALL_RIGHT-self.w then self.behavior_direction = 'LEFT' end
            if self.p.x <= ENEMY_WALL_LEFT+self.w then self.behavior_direction = 'RIGHT' end
        end
    end
}
